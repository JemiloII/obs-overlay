...
export function useAlertWebSocket(): void {
  const enqueueAlert = useAlertQueueStore((state) => state.enqueueAlert);
    ...
    function connect(): void {
      ...
      socket.addEventListener("message", (messageEvent) => {
        try {
          ...
          if (parsed.kind === "alertEvent") {
            enqueueAlert(parsed.data);
          }
        } catch (parseError) {
          console.error("Failed to parse alert message", parseError);
        }
      });
